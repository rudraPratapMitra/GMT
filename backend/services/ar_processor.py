...
# For this deployment only ECC company code US01 is constrained: every row
# tagged US01 must carry USD. A row that doesn't is flagged as a currency
# exception (see find_currency_mismatch / transform_record) rather than
# silently migrated. Extend this dict if another company code needs the
# same rule later, e.g. {"US01": "USD", "CA01": "CAD"}.
REQUIRED_CURRENCY_BY_ECC_COMPANY = {
    "US01": "USD",
}


def find_currency_mismatch(ecc_company, currency):
    """
    Returns the currency this ECC company code is required to use if
    `currency` violates it, else None. Company codes with no rule in
    REQUIRED_CURRENCY_BY_ECC_COMPANY are never flagged.
    """
    required = REQUIRED_CURRENCY_BY_ECC_COMPANY.get(ecc_company)
    if not required or not currency or currency == required:
        return None
    return required
...
class ARProcessResult:
    ...
    s4_rows: list        # transformed rows (dicts keyed by S/4 field)
    warnings: list       # empty list => safe to push to S/4
    currency_mismatches: list   # rows flagged by REQUIRED_CURRENCY_BY_ECC_COMPANY;
                                 # also included in `warnings` (code CURRENCY_MISMATCH)
                                 # so load-to-s4 stays blocked, but broken out here with
                                 # the extra detail the "Currency Exceptions" UI needs
...
def transform_record(record, index):
    ...
    # --- company code / currency sanity check ---------------------------
    ecc_currency = clean_string(record.get("WAERS")).upper()
    required_currency = find_currency_mismatch(ecc_company.upper(), ecc_currency)
    mismatch = None
    if required_currency:
        mismatch = {
            "record": index,
            "document": document,
            "company_code": ecc_company,
            "customer": ecc_customer,
            "currency": ecc_currency,
            "expected_currency": required_currency,
            "amount": clean_float(record.get("WRBTR")),
        }
        warn("WAERS", "CURRENCY_MISMATCH",
             f"Company code '{ecc_company}' should only use {required_currency}; "
             f"found '{ecc_currency}'.")
...
def transform_ecc_records(records):
    """ECC records -> (s4_rows, warnings, currency_mismatches). Pure; touches no files."""
    records = normalize_records(records)
    validate_ecc_records(records)

    s4_rows, warnings, currency_mismatches = [], [], []
    for index, record in enumerate(records, start=1):
        row, row_warnings, mismatch = transform_record(record, index)
        s4_rows.append(row)
        warnings.extend(row_warnings)
        if mismatch:
            currency_mismatches.append(mismatch)
    return s4_rows, warnings, currency_mismatches
...
def process_ar_records(records, template_path):
    ...
    Raises EccDataError (bad input) or TemplateError (bad template).
    `result.warnings` empty => the rows are clean and can be pushed to S/4.
    `result.currency_mismatches` non-empty => at least one row broke the
    company-code/currency rule (REQUIRED_CURRENCY_BY_ECC_COMPANY); the row
    is still included in `result.s4_rows` and the workbook, same as any
    other warning, until the caller explicitly removes it and reprocesses.
    """
    s4_rows, warnings, currency_mismatches = transform_ecc_records(records)
    output = build_s4_workbook(s4_rows, warnings, template_path)
